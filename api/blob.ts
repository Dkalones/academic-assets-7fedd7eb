...
export default async function handler(
  ...
  try {
    ...
    if (req.method === "POST" && action === "upload") {
      ...
      const disciplinaId = (req.query.disciplinaId as string) || "";
      const filename = (req.query.filename as string) || "";
 
      if (!disciplinaId || !filename) {
        return res.status(400).json({
          error: "disciplinaId e filename são obrigatórios",
        });
      }
 
      const pathname = `materials/${disciplinaId}/${filename}`;
      const contentType =
        (req.headers["content-type"] as string) ||
        "application/octet-stream";
 
      const result = await put(pathname, req as any, {
        access: "public",
        addRandomSuffix: false,
        allowOverwrite: true,
        contentType,
